...
MainBVSSemi <- function(
    Method = "BVSSemiMRF", LocalPrior = FALSE, Y = Y, X = X, Xcov = NULL, seed = 1,
    atheta = 1, btheta = 1, tau2cont = 1, tau2bin = 0.5, nu1cont = -3, nu2bin = -3, varpropTheta = .25,
    Bigtau2 = 100, asigma = .1, bsigma = .1, mcmcsample = 10000, burnin = 5000) {
  tau21 <- tau2cont
  tau22 <- tau2bin
  nu1 <- nu1cont
  nu2 <- nu2bin
  ...
  thetasample <- rep(0, NN - burnin)
  sigma2Sample <- rep(0, NN - burnin)
  ...
  for (s in 1:NN) {
    ...
    if ((Method == "BVSSemiIndep") || (Method == "BVSSemiMRF")) {
      ...
      Gamma1F <- SampleGammaLinear(
        GammaM1 = Gamma1, y = y2, X = X[N2, ], Xcov = Xcov[N2, ], pc = pc, asigma = asigma, bsigma = bsigma, sigma2 = sigma2, tau2 = tau21,
        Bigtau2 = Bigtau2, theta = theta, GammaM2 = Gamma2, nu = nu1, Local = LocalPrior
      )
      ...
      Gamma2F <- SampleGamma(
        ...
        Bigtau2 = Bigtau2, theta = theta, GammaM2 = Gamma1, nu = nu2, Local = TRUE
      )
